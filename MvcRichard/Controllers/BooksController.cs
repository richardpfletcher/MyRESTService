...
public ActionResult Hacks()
        ...
        public ActionResult ElectronicDoppelgänger()
        {


            LoadKeysElectronicDoppelgänger s1 = LoadKeysElectronicDoppelgänger.Instance();
            List<BookModel> items = LoadKeysElectronicDoppelgänger.list;

            String Path = Server.MapPath("/Audio/Books/ElectronicDoppelgänger");
            String[] FileNames = Directory.GetFiles(Path);
            ...
            foreach (var data in items) //iterate the file list
            {
                foreach (string path in FileNames) //iterate the file list
                {
                    ...
                    if (shortname.ToUpper() == data.Chapter.ToUpper())
                    {
                        list.Add(new DocumentModel(fullname, shortname, "\\Audio\\Books\\ElectronicDoppelgänger\\" + fullname, "http://www.evolutionrevolutionoflove.com/Audio/Books/ElectronicDoppelgänger/" + fullname));
                        break;
                    }
                }
            }
        ...
        public ActionResult Blood()
        {
            ...
            LoadKeysBlood s1 = LoadKeysBlood.Instance();
            List<BookModel> items = LoadKeysBlood.list;

            String Path = Server.MapPath("/Audio/Books/Blood");
            String[] FileNames = Directory.GetFiles(Path);
            ...
            foreach (var data in items) //iterate the file list
            {
                foreach (string path in FileNames) //iterate the file list
                {
                    ...
                    if (shortname.ToUpper() == data.Chapter.ToUpper())
                    {
                        list.Add(new DocumentModel(fullname, shortname, "\\Audio\\Books\\Blood\\" + fullname, "http://www.evolutionrevolutionoflove.com/Audio/Books/Blood/" + fullname));
                        break;
                    }
                }
            }
